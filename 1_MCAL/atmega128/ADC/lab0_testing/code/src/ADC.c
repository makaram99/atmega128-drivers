...
ERROR_t ADC_Read(const ADC_CHANNEL_t channel, u16_t * const ptrToValue) {
    ...
    if( ASSERT_VALID_ADC(adcConfigs[i].adc)) {
        ...
        if(BIT_IS_SET(ADC->ADCSRA, ADIE)) {
            /*!< ISR will handle the reading    */
            error |= ERROR_BUSY;
        } else {
...
ERROR_t ADC_SetCallback(const ADC_CHANNEL_t channel, void (* const ptrToCallback)(void)) {
    ERROR_t error = ERROR_OK;
    s8_t i = 0;
    ...
    if(error | (i < 0)) {
        ...
        adcConfigs[i].callback = ptrToCallback;
    }
...
static ERROR_t ADC_InitChannel(const ADC_CHANNEL_t channel) {
    ...
    if(NULL != adcConfigs[i].callback) {
        ADC_SetCallback(channel, adcConfigs[i].callback);
        ADC_EnableInterrupt();
    } else {
        /* Do nothing */
    }    

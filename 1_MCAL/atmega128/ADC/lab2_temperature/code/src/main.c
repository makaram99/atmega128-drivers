...
int main (void){   
	...
	while(1) {
        ...
        ADC_Read(ADC_CHANNEL_TEMPERATURE, &temperatureInBinary);
        CLCD_WriteStringInXY("Binary: ", CLCD_LINE_1, 0);
        CLCD_WriteIntegerInXY(temperatureInBinary, CLCD_LINE_1, 8);
        ...
        temperatureInCelsius = ((temperatureInBinary * 5.0f) / 1024) * 100;

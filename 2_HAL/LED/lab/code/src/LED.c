...
/*                                                                              */
/*------------------------------------------------------------------------------*/
#define ASSERT_LED(led)         ( (led < NUM_OF_LEDS) && (led >= LED_0) )
...
ERROR_t LED_SetClr(const LED_t led, const STATE_t state) { 
    ...
    if(i >= 0) {
        error |= DIO_SetPinValue(ledConfigs[i].pin, ledConfigs[i].port, state);
    } else {
...
ERROR_t LED_Toggle(const LED_t led) {
    ...
    error |= DIO_ReadPin(ledConfigs[i].pin, ledConfigs[i].port, &state);

    /* Toggle the LED */
    DIO_SetPinValue(ledConfigs[i].pin, ledConfigs[i].port, !state);
...
ERROR_t LED_Read(const LED_t led, STATE_t * const ptrToState) { 
    ...
    error |= LED_ReadIndex(led, &i);
    if(i >= 0) {
        error |= DIO_ReadPin(ledConfigs[i].pin, ledConfigs[i].port, ptrToState);
    } else {
        error |= ERROR_INVALID_PARAMETER;
    }

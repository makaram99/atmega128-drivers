...
ERROR_t CLCD_Init(void) {
    ...
    error |= CLCD_Send(CLCD_DISPLAY_MODE, SEND_MODE_COMMAND);
...
ERROR_t CLCD_WriteInteger(const s32_t number) {
    ...
    u8_t digit = 0, multiplier = 0;
    u8_t count = 0, i = 0;

    if(number < 0) {
        CLCD_WriteChar('-');
        Loc_s32Number = -number;
    }
    ...
    /* Write the number */
    Loc_s32Number = (number >= 0 ? number : -number);
    while(count) {
        multiplier = 1;
        while(i < (count - 1)) {
            multiplier *= 10;
        ...
        digit = Loc_s32Number / multiplier;
        Loc_s32Number %= multiplier;

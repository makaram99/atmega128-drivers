...
ERROR_t CLCD_Init(void) {
    ...
    #if (MODE_4_BITS_USED)
        /* Set number of lines and dots height */
        #if ((CLCD_NUMBER_OF_LINES == 2) && (CLCD_DOTS_HEIGHT == 8))
        error |= CLCD_SendCommand(CLCD_CMD_4_BIT_2_LINES_8_DOTS);
        #elif ((CLCD_NUMBER_OF_LINES == 2) && (CLCD_DOTS_HEIGHT == 11))
        error |= CLCD_SendCommand(CLCD_CMD_4_BIT_2_LINES_11_DOTS);
        #elif ((CLCD_NUMBER_OF_LINES == 1) && (CLCD_DOTS_HEIGHT == 8))
        error |= CLCD_SendCommand(CLCD_CMD_4_BIT_1_LINES_8_DOTS);
        #elif ((CLCD_NUMBER_OF_LINES == 1) && (CLCD_DOTS_HEIGHT == 11))
        error |= CLCD_SendCommand(CLCD_CMD_4_BIT_1_LINES_11_DOTS);
        #endif
    #else
        /* Set number of lines and dots height */
        #if ((CLCD_NUMBER_OF_LINES == 2) && (CLCD_DOTS_HEIGHT == 8))
        error |= CLCD_SendCommand(CLCD_CMD_8_BIT_2_LINES_8_DOTS);
        #elif ((CLCD_NUMBER_OF_LINES == 2) && (CLCD_DOTS_HEIGHT == 11))
        error |= CLCD_SendCommand(CLCD_CMD_8_BIT_2_LINES_11_DOTS);
        #elif ((CLCD_NUMBER_OF_LINES == 1) && (CLCD_DOTS_HEIGHT == 8))
        error |= CLCD_SendCommand(CLCD_CMD_8_BIT_1_LINES_8_DOTS);
        #elif ((CLCD_NUMBER_OF_LINES == 1) && (CLCD_DOTS_HEIGHT == 11))
        error |= CLCD_SendCommand(CLCD_CMD_8_BIT_1_LINES_11_DOTS);
        #endif
    #endif

    error |= CLCD_SendCommand(CLCD_CMD_DISPLAY_ON_CURSOR_OFF_BLINK_OFF);
    error |= CLCD_SendCommand(CLCD_CMD_CLEAR_DISPLAY);
    error |= CLCD_SendCommand(CLCD_CMD_CURSOR_INCREMENT_NO_DISPLAY_SHIFT);

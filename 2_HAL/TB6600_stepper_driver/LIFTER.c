...
static void LIFTER_SetDirection(const LIFTER_DIR_t dir) {
    if(LIFTER_UP == dir) {
        DIO_SetPinValue(LifterConfigs.directionPin, LifterConfigs.directionPinPort, HIGH);
    }
    else {
        DIO_SetPinValue(LifterConfigs.directionPin, LifterConfigs.directionPinPort, LOW);
    }
}
...
static void LIFTER_SendPulses(const u32_t pulsesPerSecond, const u8_t overallSeconds) {
    ...
    for(u32_t i = 0; i < (pulsesPerSecond * overallSeconds); ++i) {
        DIO_SetPinValue(LifterConfigs.pulsePin, LifterConfigs.pulsePinPort, HIGH);
        _delay_us(pulseDelay);
        DIO_SetPinValue(LifterConfigs.pulsePin, LifterConfigs.pulsePinPort, LOW);
        _delay_us(pulseDelay);
    }
}
...
void LIFTER_Enable(void) {
    DIO_SetPinValue(LifterConfigs.enablePin, LifterConfigs.enablePinPort, HIGH);
}
...
void LIFTER_Disable(void) {
    DIO_SetPinValue(LifterConfigs.enablePin, LifterConfigs.enablePinPort, LOW);
}
